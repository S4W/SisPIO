						...
						estado[0].lower() == "Estado:".lower()): # Verificamos que la cabecera y la linea del liceo tenga el formato correcto
							if ((estado[1].lower()=="Pre-inscrito".lower()) or (estado[1].lower()=="Seleccionado".lower()) or
							   (estado[1].lower()=="Activo".lower()) or (estado[1].lower()=="Inactivo".lower()) or
								...
								response.flash = "El estado seleccionado es invalido. Los estados disponibles son Pre-inscrito, Seleccionado, Activo, Inactivo, Finalizado"
						else:
							response.flash = "Formato de los datos del archivo inválido. Consulte el manual"             # Error de Carga
					else:
						response.flash = len(texto)             # Error de Carga
...
def consultarUsuarios():
	...
	if request.vars:
		...
		if request.vars.tipoUsuario=="administrador":
			...
			# Orden
			orden = None
			if request.vars.tipoOrdenA=="cedula":
				orden = db.usuario.username
			elif request.vars.tipoOrdenA=="nombre":
		...
		#########################
			...
			if request.vars.tipoOrdenE == "cedula":
				orden = db.usuario.username
			elif request.vars.tipoOrdenE == "institucion":
		...
		######################
			...
			# Orden
			orden = None
			if request.vars.tipoOrdenP=="cedula":
				orden = db.usuario.username
			elif request.vars.tipoOrdenP=="materia":
		...
		#####################################
			...
			# Filtros
			if request.vars.institucion != "Todos":
				query = query & (db.representante_liceo.nombre_liceo==request.vars.institucion)
			if request.vars.tipo != "Todos":
				query = query & (db.liceo.tipo==request.vars.tipo)

			# Orden
			orden = None
			if request.vars.tipoOrdenL=="institucion":
				orden = db.representante_liceo.nombre_liceo
			elif request.vars.tipoOrdenL=="tipoInstitucion":
				orden = db.liceo.tipo
		...
		####################################
			...
			# Orden
			orden = None
			if request.vars.tipoOrdenS=="cedula":
...
def resultadosConsulta():
	...
	if tipoUsuario == 'estudiante':
		for c in consulta:
			ci = c.usuario["username"]
			notaTI = db((db.resultados_prueba.ci_estudiante == ci) & (db.resultados_prueba.id_examen == "Test de Inteligencia")).select()
			notaHM = db((db.resultados_prueba.ci_estudiante == ci) & (db.resultados_prueba.id_examen == "Habilidad Matematica")).select()
			notaHV = db((db.resultados_prueba.ci_estudiante == ci) & (db.resultados_prueba.id_examen == "Habilidad Verbal")).select()
			notaTotal = 0.0

			if notaTI:
				notaTI = notaTI.first()["resultado"]
				notaTotal += float(notaTI)*0.4
			else:
				notaTI = "-"
			if notaHM:
				notaHM = notaHM.first()["resultado"]
				notaTotal += float(notaHM)*0.3
			else:
				notaHM = "-"
			if notaHV:
				notaHV = notaHV.first()["resultado"]
