...
db.define_table(
    'exime',
    Field('ci_estudiante', type='reference usuario.username', length=8, notnull=True, requires=[IS_IN_DB(db, db.estudiante.ci)]),
    Field('ci_representante_liceo', type='reference usuario.username', length=8, notnull=True, requires=[IS_IN_DB(db, db.representante_liceo.ci)]),
